...
export const Round = ({ style, size, source, imageStyle }) => {
    const defaultSize = totalSize(5)
    return (
        <Image
            source={source}
            resizeMode={'contain'} imageStyle={imageStyle}
...
export const Primary = ({ style, size, source, resizeMode, onPress }) => {
    const defaultSize = totalSize(5)
    return (
        <TouchableOpacity onPress={onPress} activeOpacity={1}>

            <Image resizeMode={resizeMode ? resizeMode : 'contain'}
                source={source}
